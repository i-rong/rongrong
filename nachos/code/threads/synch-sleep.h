...
class Lock {
  ...
  private:
    ...
    bool state; // record the lock is locked or not
    List* queue; //  save the threads that are waiting
    Thread* threadHoldLock; // the thread that hold the lock, only the thread can release the lock
...
class Condition {
  ...
  private:
    char* name;
    List* queue; // store suspended threads
    // plus some other stuff you'll need to define
};
#endif // SYNCH_H
